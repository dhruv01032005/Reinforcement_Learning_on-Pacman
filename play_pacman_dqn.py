...
def select_checkpoint():
    ...
    checkpoint_dir = Path("checkpoints_pacman_dqn")
    ...
    if not checkpoints:
        print("❌ No checkpoint files found in checkpoints_pacman_dqn/")
        sys.exit(1)
...
def draw_overlay(screen, score, episode_num, font, small_font, paused=False, game_over=False):
    ...
    score_bg = pygame.Rect(10, y_pos, 180, 35)
    ...
    # Episode info
    dqn_text = small_font.render(f"DQN", True, WHITE)
    ep_text = small_font.render(f"Episode {episode_num}", True, WHITE)
    screen.blit(dqn_text, (SCREEN_WIDTH - 180, y_pos + 0))
    ...
    if paused or game_over:
        ...
        if paused:
            ...
            screen.blit(pause_text, pause_text.get_rect(center=(SCREEN_WIDTH // 2, GAME_HEIGHT // 2)))
        elif game_over:
            go_text = font.render("GAME OVER", True, RED)
            fs_text = small_font.render(f"Final Score: {score}", True, YELLOW)
            screen.blit(go_text, go_text.get_rect(center=(SCREEN_WIDTH // 2, GAME_HEIGHT // 2 - 30)))
            screen.blit(fs_text, fs_text.get_rect(center=(SCREEN_WIDTH // 2, GAME_HEIGHT // 2 + 20)))
