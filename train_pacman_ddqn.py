...
# -------------------- Setup --------------------
CHECKPOINT_DIR = Path("checkpoints_pacman_ddqn")
METRICS_DIR = Path("metrics_pacman_ddqn")
for d in [CHECKPOINT_DIR, METRICS_DIR]:
    d.mkdir(exist_ok=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")
print(f"Algorithm: Double DQN (DDQN)")
...
class DoubleDQNAgent:
    ...
    def optimize_model(self):
        """
        Double DQN optimization - key difference from vanilla DQN
        """
...
def train():
    ...
    print(f"\n🎮 Starting Pac-Man Double DQN Training for {num_episodes} episodes")
    print(f"💡 Double DQN uses policy net for action selection + target net for evaluation\n")
