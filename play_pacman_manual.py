...
def draw_overlay(screen, score, font, small_font, paused=False, game_over=False,
    ...
    # Position UI elements below the game area
    ...
    if ep_text_surface:
        screen.blit(ep_text_surface, (SCREEN_WIDTH - 180, y_pos + 15))
    ...
    if paused or game_over:
        ...
        if paused:
            if paused_surf:
                screen.blit(paused_surf, paused_surf.get_rect(center=(SCREEN_WIDTH // 2, GAME_HEIGHT // 2)))
        elif game_over:
            if gameover_surf:
                screen.blit(gameover_surf, gameover_surf.get_rect(center=(SCREEN_WIDTH // 2, GAME_HEIGHT // 2 - 30)))
            if final_score_surf:
                screen.blit(final_score_surf, final_score_surf.get_rect(center=(SCREEN_WIDTH // 2, GAME_HEIGHT // 2 + 20)))
